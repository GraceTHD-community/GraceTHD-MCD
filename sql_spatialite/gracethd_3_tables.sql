...
CREATE TABLE t_organisme(	or_code VARCHAR (20) NOT NULL  ,
	...
	or_ad_code VARCHAR(254)   REFERENCES t_adrese(ad_code),
...
CREATE TABLE t_noeud(	nd_id INTEGER   ,
	...
	nd_ad_code VARCHAR(254)   REFERENCES t_adresse(ad_code),
	nd_voie VARCHAR(254)   ,
	nd_type VARCHAR(1)   REFERENCES l_noeud_type (code),
...
CREATE TABLE t_cheminement(	cm_id INTEGER   ,
	...
	cm_cm1 VARCHAR(254)   REFERENCES t_cheminement(cm_code),
	cm_cm2 VARCHAR(254)   REFERENCES t_cheminement(cm_code),
	...
	cm_cddispo INTEGER   ,
	cm_mod_pos VARCHAR(254)   REFERENCES l_pose_type(code),
	cm_passage VARCHAR(254)   REFERENCES l_passage_type(code),
...
CREATE TABLE t_conduite(	cd_id INTEGER   ,
	cd_code VARCHAR(254) NOT NULL UNIQUE ,
	cd_codeext Varchar(254)   ,
	cd_etiquet VARCHAR(254)   ,
	cd_cd_code VARCHAR(254)   REFERENCES t_fourreau (cd_code),
	...
	cd_dateman DATE   ,
	cd_datemes Date   ,
	cd_type VARCHAR(254) NOT NULL  REFERENCES l_conduite_type (code),
	cd_dia_int INTEGER   ,
	cd_dia_ext INTEGER   ,
...
CREATE TABLE t_cond_chem(	dm_cd_code VARCHAR(254) NOT NULL  REFERENCES t_cable(cb_code),
	dm_cm_code VARCHAR(254) NOT NULL  REFERENCES t_conduite(cd_code),
...
CREATE TABLE t_cable(	cb_id INTEGER   ,
	...
	cb_user VARCHAR(254)   REFERENCES t_organisme (or_code),
...
CREATE TABLE t_love(	lv_id INTEGER NOT NULL  ,
...
CREATE TABLE t_position(	ps_id INTEGER   ,
	ps_code VARCHAR(254) NOT NULL UNIQUE ,
	ps_numero INTEGER   ,
	ps_amont VARCHAR (254)   REFERENCES t_fibre (fo_code),
	ps_aval VARCHAR (254)   REFERENCES t_fibre (fo_code),
	ps_ca_id INTEGER   REFERENCES t_cassette (cs_code),
	ps_ti_code VARCHAR(254)   REFERENCES t_tiroir (ti_code),
	ps_type VARCHAR(10)   REFERENCES l_position_type (code),
	ps_fonct VARCHAR(3)   REFERENCES l_position_fonction (code),
	ps_etat VARCHAR(2)   REFERENCES l_etat_type (code),
...
CREATE TABLE t_ropt(	rt_id INTEGER   ,
	...
	rt_fo_code INTEGER   REFERENCES t_fibre (fo_code),
...
CREATE TABLE t_ptech(	pt_id INTEGER   ,
	...
	pt_dateins DATE   ,
	pt_datemes Date   ,
	pt_typephy VARCHAR(20) NOT NULL  REFERENCES l_ptech_type_phy (code),
	pt_typelog VARCHAR(1) NOT NULL  REFERENCES l_ptech_type_log (code),
	pt_nature VARCHAR (1)   REFERENCES l_ptech_nature (code),
	pt_secu BOOLEAN   ,
...
CREATE TABLE t_ebp(	bp_id INTEGER   ,
	...
	bp_sf_code VARCHAR(50)   REFERENCES t_suf(sf_code),
...
CREATE TABLE t_suf(	sf_id INTEGER   ,
	...
	sf_ad_code INTEGER   REFERENCES t_adresse (ad_code),
	sf_escal VARCHAR (20)   ,
	sf_etage VARCHAR (20)   ,
...
CREATE TABLE t_adresse(	ad_id INTEGER   ,
	...
	ad_isole BOOLEAN   ,
	ad_prio BOOLEAN   ,
	ad_racc VARCHAR(1)   REFERENCES l_implantation_type(code),
	ad_baticode VARCHAR(100)   ,
	ad_nombat VARCHAR(254)   ,
	ad_ietat INTEGER   REFERENCES l_adresse_etat(code),
...
CREATE TABLE t_siteemission(	se_id INTEGER NOT NULL  ,
	...
	se_haut NUMERIC(5,2)   ,
	se_ad_code INTEGER   REFERENCES t_adrese(ad_code),
...
CREATE TABLE t_znro(	zn_id INTEGER   ,
	zn_code VARCHAR(254)  UNIQUE ,
...
CREATE TABLE t_zsro(	zs_id INTEGER   ,
	zs_code VARCHAR(254)  UNIQUE ,
	...
	zs_etatpm VARCHAR(2)   REFERENCES l_zsro_etat(code),
	zs_dateins DATE   ,
	zs_typeemp VARCHAR(3)   REFERENCES l_zsro_emplacement(code),
	zs_capamax INTEGER   ,
	zs_ad_code VARCHAR(100)   REFERENCES t_adrese(ad_code),
...
CREATE TABLE t_zpbo(	zp_id INTEGER   ,
	zp_code VARCHAR(254)  UNIQUE ,
	zp_nd_code VARCHAR(254) NOT NULL  REFERENCES t_noeud (nd_code),
	zp_zs_code VARCHAR(254)   REFERENCES t_zone (zs_code),
...
CREATE TABLE t_zdep(	zd_id INTEGER   ,
	zd_code VARCHAR(254)  UNIQUE ,
	zd_nd_code VARCHAR(254)   REFERENCES t_noeud (nd_code),
	zd_zs_code VARCHAR(254)   REFERENCES t_zone (zs_code),
...
CREATE TABLE t_zcoax(	zc_id INTEGER   ,
	zc_code VARCHAR(254)  UNIQUE ,
...
CREATE TABLE t_docobj(	od_id INTEGER NOT NULL  ,
	od_do_ref VARCHAR(254)   ,
	od_tbltype VARCHAR(10)   REFERENCES l_doc_tab (code),
	od_codeobj VARCHAR5254)   ,
...
CREATE TABLE t_empreinte(	em_id INTEGER NOT NULL  ,
	em_code VARCHAR(254)  UNIQUE ,
	em_do_ref VARCHAR(254)   ,
